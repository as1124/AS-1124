...
function Line(x1,y1,x2,y2) {
	...
	if (dx >= dy)
	{
		...
		while ((dx--) > 0)
		{
			...
			if (p > 0)
			{
				this.mkDiv(ox, y, x-ox, lineWidth);
				y += yIncr;
				p += pru;
				ox = x;
			}
		...
		this.mkDiv(ox, y, x2-ox+1, lineWidth);
	}
	...
	else
	{
		...
		if (y2 <= y1)
		{
			while ((dy--) > 0)
			{
				if (p > 0)
				{
					this.mkDiv(x++, y, lineWidth, oy-y+1);
					y += yIncr;
					p += pru;
					oy = y;
				}
			...
			this.mkDiv(x2, y2, lineWidth, oy-y2+1);
		}
		else
		{
			while ((dy--) > 0)
			{
				...
				if (p > 0)
				{
					this.mkDiv(x++, oy, lineWidth, y-oy);
					p += pru;
					oy = y;
				}
				else p += pr;
			}
			this.mkDiv(x2, oy, lineWidth, y2-oy+1);
		}
	}
} 
