...
public class HttpExecuter {
	 ...
	 * GET请求
	 * @param httpClient
	 * @param url 完整URL，包含请求参数。如 <code>https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=APPID&secret=APPSECRET</code>
	 * @param reffer
	 * @param cookie
	 ...
	 * POST请求
	 * @param httpClient
	 * @param url 完整URL，包含查询参数，如 <code>https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=APPID&secret=APPSECRET</code>
	 * @param entity POST请求报文数据对象
	 * @param reffer
	 * @param cookie
	 * @param socketTimeout socket超时时间
	 * @param connectTimeout 连接超时时间
	 ...
	 * @param url 请求完整URL，包含查询参数，如 <code>https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=APPID&secret=APPSECRET</code>
	 * @param localFilePath 待上传文件的本地路径
	 * @param partName 表单名称（key）
	 * @return
	 */
	@Bizlet("executeUploadFile")
